    ...
    public partial class Form4 : Form
    {
            ...
            using (SqlConnection conn = new SqlConnection(strKonek))
            {
                try
                {
                    ...
                    string query = @"SELECT ID_Donasi AS [ID Donasi], ID_Donatur AS [ID Donatur], 
                                     Jenis_Barang AS [Jenis Barang], Nama_Barang AS [Nama Barang], Jumlah AS [Jumlah] FROM Donasi";
        ...
        private void btnTambah_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(strKonek))
            {
                ...
                {
                    conn.Open();
                    string query = "INSERT INTO Donasi (ID_Donasi, ID_Donatur, Jenis_Barang, Nama_Barang, Jumlah) VALUES (@ID, @Donatur, @Jenis, @Nama_Barang, @Jumlah)";
                    ...
                    cmd.Parameters.AddWithValue("@Nama_Barang", txtNamaBarang.Text.Trim());
        ...
        private void btnUbah_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Isi ID Donasi dan pilih ID Donatur.", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (cmbJenisBarang.SelectedIndex == -1)
            {
                MessageBox.Show("Pilih jenis barang!", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            using (SqlConnection conn = new SqlConnection(strKonek))
            {
                try
                {
                    string query = "UPDATE Donasi SET ID_Donatur=@Donatur, Jenis_Barang=@Jenis, Nama_Barang=@Nama_Barang, Jumlah=@Jumlah WHERE ID_Donasi=@ID";
                    SqlCommand cmd = new SqlCommand(query, conn);
